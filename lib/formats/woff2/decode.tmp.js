...
function decodeTableData(fontData, srcReader, state) {
  ...
  return series.then(function () {
    ...
    // types 255Uint16 and UintBase128, which are variable-length, those are not present in the
    // OpenType format. That means, after reconstructing the data, the result *should* always be
    // byte-equal to the original OpenType font. Therefore, it should be safe to allocate just
    // totalSfntSize (minus size of non-transformed tables) bytes.
