...
var identity = utils.identity = function (val) {
  return function () {
    return val;
  };
};

utils.nullIdentity = identity(null);
...
utils.detectFontOutlineType = function (sfntVersion, tablesByTag) {
  ...
  var isTrueType = sfntVersion === constants.FLAVOR_TRUETYPE ||
    sfntVersion === constants.FLAVOR_TRUETYPE_APPLE;

  if (isTrueType && hasGlyf && hasLoca) {
